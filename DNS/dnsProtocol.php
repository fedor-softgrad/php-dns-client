...
namespace Metaregistrar\DNS {
    class dnsProtocol
    {
        ...
        function __construct($logging = false)
        {
            ...
            $this->port=53;
            $this->setConnectionTimeout(self::DEFAULT_CONNECTION_TIMEOUT);
            $this->setReadTimeout(self::DEFAULT_READ_TIMEOUT);
            $this->udp=false;
        ...
        /**
         * @param float $timeout - the integer part of number means seconds whereas fractional part of the number means microseconds
         * @throws dnsException
         */
        public function setConnectionTimeout($timeout = self::DEFAULT_CONNECTION_TIMEOUT)
        {
            ...
            {
                throw new dnsException("Incorrect connection timeout value: <" . $timeout . ">. Timeout must be positive number.");
            }
            $this->connectiontimeout = $new_timeout;
        }
        ...
        public function setReadTimeout($timeout = self::DEFAULT_READ_TIMEOUT)
        {
            if (!$new_timeout = floatval($timeout) || $timeout < 0)
            {
                throw new dnsException("Incorrect read timeout value: <" . $timeout . ">. Timeout must be positive number.");
            }
            $this->readtimeoutsec = (int)floor($new_timeout); // get seconds
            $this->readtimeoutusec = (int)round(($new_timeout - $this->readtimeoutsec) * 1000000); // get microseconds
        }
